...
function reportRuntimeError(source, error) {
  ...
  console.error(`RUNTIME ERROR (${source}):`, err);
  ...
  showModal({
    ...
    content: `<p>An unexpected error occurred while running the app.</p><p class="mt-2 text-sm text-gray-500">${err.message}</p>`,
...
function safeRun(source, fn) {
  try {
    const result = fn();
    if (result && typeof result.then === 'function') {
      result.catch((error) => reportRuntimeError(source, error));
    }
    return result;
  } catch (error) {
    reportRuntimeError(source, error);
    return null;
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (SUPABASE_URL.includes('YOUR_SUPABASE_URL')) {
    ...
    if (contentWrapper)
      contentWrapper.innerHTML = `<div class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mt-8" role="alert"><p class="font-bold">Configuration Error</p><p>Please update the <strong>SUPABASE_URL</strong> and <strong>SUPABASE_ANON_KEY</strong> in the main.js file.</p></div>`;
    return;
  }
...
export function handleDataLoad(data, isInitial = true) {
  ...
  if (!data.full_name) {
    ...
    safeRun('renderAccountPage.setup', () => renderAccountPage(true));
  } else {
    ...
    if (isAccountPageVisible && !isInitial) {
      ...
      safeRun('renderAccountPage.refresh', () => renderAccountPage(false));
    } else {
...
function backupData() {
  ...
  if (!appState.gradebook_data) {
    showModal({
      title: 'Backup Failed',
      content: `<p>No data found to create a backup file.</p>`,
      confirmText: null,
      cancelText: 'Close',
      modalWidth: 'max-w-xs',
    });
  ...
  try {
    ...
    showModal({
      ...
      content: `<p>Your data has been downloaded as <strong>${a.download}</strong>.</p>`,
      confirmText: null,
      cancelText: 'Close',
      modalWidth: 'max-w-xs',
    });
    ...
    showModal({
      title: 'Backup Failed',
      content: `<p>Could not create backup file due to an error.</p>`,
      confirmText: null,
      cancelText: 'Close',
      modalWidth: 'max-w-xs',
    });
    ...
    reader.onload = function (event) {
      try {
          ...
          showModal({
            ...
            onConfirm: () => {
              ...
              showModal({
                ...
                content: `<p>Data restored successfully!</p>`,
                confirmText: null,
                cancelText: 'Close',
                modalWidth: 'max-w-xs',
              });
        ...
        showModal({
          ...
          content: `<p>Error reading backup file: ${error.message}</p>`,
          confirmText: null,
          cancelText: 'Close',
          modalWidth: 'max-w-sm',
        });
      ...
      if (input && input.value === 'DELETE') {
        try {
          ...
          showModal({
            ...
            content: '<p>Your account has been permanently deleted.</p>',
            confirmText: null,
            cancelText: 'Close',
          });
          signOut(supabaseClient, true);
          ...
          showModal({
            title: 'Error',
            content: `<p>Could not delete account: ${error.message}</p>`,
            confirmText: null,
            cancelText: 'Close',
          });
        }
        ...
        showModal({
          ...
          content: '<p>The confirmation text did not match. Your account is safe.</p>',
          confirmText: null,
          cancelText: 'Close',
          modalWidth: 'max-w-xs',
        });
...
function setupEventListeners() {
  ...
  if (contentWrapper) {
    contentWrapper.addEventListener('click', (e) => {
      ...
      if (studentNameBtn) {
        ...
        if (studentId) {
          safeRun('renderStudentProfileModal', () => renderStudentProfileModal(studentId));
          return;
        }
      }
      ...
      if (id === 'back-to-app-btn') {
        safeRun('handleDataLoad', () => handleDataLoad(getAppState(), true));
        return;
      }
      if (id === 'save-profile-btn') {
        safeRun('saveProfile', () => saveProfile());
        return;
      }
      if (id === 'delete-account-btn') {
        safeRun('promptDeleteAccount', () => promptDeleteAccount());
        return;
      }
      ...
      const actionMap = {
        ...
        exportBlankPdfBtn: () => {
          actions.exportBlankMarksheet();
          document.getElementById('exportMenuDropdown')?.classList.add('hidden');
        },
        exportStudentListBtn: () => {
          actions.exportStudentListPDF();
          document.getElementById('exportMenuDropdown')?.classList.add('hidden');
        },
        exportContactListBtn: () => {
          actions.exportContactListPDF();
          document.getElementById('exportMenuDropdown')?.classList.add('hidden');
        },
      ...
      if (actionMap[id]) {
        e.preventDefault();
        safeRun(`action:${id}`, () => actionMap[id]());
      } else if (tabId) {
        e.preventDefault();
        safeRun('switchActiveClass', () => actions.switchActiveClass(tabId));
      }
    });
    ...
    contentWrapper.addEventListener('change', (e) => {
      ...
      if (target.id === 'attendance-date-picker') {
        safeRun('renderAttendanceSheet', () => renderAttendanceSheet(target.value));
        return;
      }

      if (target.id === 'show-archived-checkbox') {
        safeRun('renderClassTabs', () => renderClassTabs());
        return;
      }
      ...
      if (target.id === 'unitFilterDropdown') {
        ...
        if (appState.gradebook_data) {
          ...
          safeRun('renderGradebook.unitFilter', () => renderGradebook());
          triggerAutoSave();
        }
        return;
      }
    ...
    contentWrapper.addEventListener('input', (e) => {
      ...
      if (target.classList.contains('cat-name-input')) {
        ...
        if (classData && cat) {
          ...
          safeRun('renderGradebook.categoryRename', () => renderGradebook());
          triggerAutoSave();
        }
        return;
      }
      ...
      if (target.id === 'student-search-input') {
        safeRun('renderGradebook.search', () => renderGradebook());
        return;
      }
      ...
      if (target.classList.contains('grade-input')) {
        ...
        if (studentId && assignmentId && classData?.students?.[studentId]) {
          ...
          const updateCell = (cat, val) => {
            if (cat) {
              classData.students[studentId].grades[assignmentId][cat] = val;
            } else {
              classData.students[studentId].grades[assignmentId].grade = val;
            }
          ...
          if (unit?.isFinal) {
            maxScore = assignment?.total ?? Infinity;
          } else if (category) {
            maxScore = assignment?.categoryTotals?.[category] ?? Infinity;
          }
  ...
  ['mousemove', 'mousedown', 'keypress', 'click'].forEach((event) =>
...
function handleModalEnter(modal, currentInput) {
  ...
  const inputs = Array.from(
    modal.querySelectorAll('input:not([type="hidden"]):not([disabled]), select:not([disabled])')
  );
