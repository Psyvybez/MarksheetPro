...
export function calculateClassStats(classData) {
  ...
  students.forEach((student) => {
    ...
    // Unit-level rollups intentionally omitted for now.
  });
...
export function recalculateAndRenderAverages() {
  ...
  students.forEach((student) => {
    ...
    if (avgs.overallGrade !== null) {
      classOverallSum += avgs.overallGrade;
      classOverallCount++;
    }
    if (avgs.termMark !== null) {
      classTermSum += avgs.termMark;
      classTermCount++;
    }
    if (avgs.finalMark !== null) {
      classFinalSum += avgs.finalMark;
      classFinalCount++;
    }
  });
